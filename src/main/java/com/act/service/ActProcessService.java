...
public class ActProcessService {
	...
	public Page<Object[]> processList(Page<Object[]> page, String category) {
	    ...
	    List<ProcessDefinition> processDefinitionList = processDefinitionQuery.listPage(page.getOffset(), page.getLimit());
	    for (ProcessDefinition processDefinition : processDefinitionList) {
	      String deploymentId = processDefinition.getDeploymentId();
	      Deployment deployment = repositoryService.createDeploymentQuery().deploymentId(deploymentId).singleResult();
	      page.getRows().add(new Object[]{processDefinition, deployment});
	    }
	...
	public Page<ProcessInstance> runningList(Page<ProcessInstance> page, String procInsId, String procDefKey) {
	    ...
	    page.setRows(processInstanceQuery.listPage(page.getOffset(), page.getLimit()));
		return page;
	}
...
//					ActUtils.exportDiagramToFile(repositoryService, processDefinition, exportDir);
	...
	public List<ComboVo> getProcessKeys() throws Exception {
		...
		for (ProcessDefinition processDefinition : definitions) {
			ComboVo map = new ComboVo();
			map.setName(processDefinition.getName());
			map.setValue(processDefinition.getKey());
			list.add(map);
		}
		return list;
	}

	public List<ComboVo> getProcessTasks(String processKey) {
		List<ComboVo> list = new ArrayList<>();
		...
		for (ActivityImpl activityImpl : activityImpls) {
			...
			if("userTask".equals(type)){
				comboVo.setName(activityImpl.getProperty("name").toString());
				comboVo.setValue(activityImpl.getId());
				list.add(comboVo);
			}
		}
		return list;
